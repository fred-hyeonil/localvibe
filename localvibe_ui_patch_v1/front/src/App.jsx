...
const NAV_ITEMS = [
  ...
  },
  {
    section: '정보',
    items: [
      { id: 'about',   label: '💡 서비스 소개', tab: 'gallery' },
      { id: 'contact', label: '📬 문의하기',    tab: 'gallery' },
    ],
  },
];
...
function pickFeedItems(items, size = FEED_SIZE) {
  ...
  for (const item of shuffled) {
    ...
    if (imageKey && usedImageKeys.has(imageKey)) continue;
    picked.push(item);
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function fetchRegionInsight() {
      ...
      try {
        ...
        const data = await response.json();
        if (isMounted && data?.region) setInsightRegion(data.region);
      } catch {
  ...
  const handleRecommendFeed = recommendedIds => {
    ...
    for (const item of selected) {
      ...
      if (uniqueSelected.length >= FEED_SIZE) break;
    }
              ...
              <div key={group.section} className="sidebar-group">
                ...
                <ul className="sidebar-list">
                  {group.items.map(item => (
                    <li key={item.id}>
                      <button
                        className={`sidebar-item ${activeTab === item.tab && item.id === activeTab ? 'active' : ''} ${item.id === 'gallery' && activeTab === 'gallery' ? 'active' : ''} ${item.id === 'planner' && activeTab === 'planner' ? 'active' : ''} ${item.id === 'mypage' && activeTab === 'mypage' ? 'active' : ''}`}
          ...
          {activeTab === 'gallery' && (
            <div className="gallery-page">
              <div className="gallery-page-header">
                <h1 className="gallery-page-title">지역 갤러리</h1>
                <p className="gallery-page-sub">광주·전남의 숨겨진 명소를 AI로 발견해보세요</p>
              </div>
